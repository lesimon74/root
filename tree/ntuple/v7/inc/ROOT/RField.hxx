...
public:
   static std::string MyTypeName() { return "float24_t"; }
   explicit RField(std::string_view name)
     : Detail::RFieldBase(name, MyTypeName(), ENTupleStructure::kLeaf, true /* isSimple */) {}
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
...
public:
   static std::string MyTypeName() { return "float16_t"; }
   explicit RField(std::string_view name)
     : Detail::RFieldBase(name, MyTypeName(), ENTupleStructure::kLeaf, true /* isSimple */) {}
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
...
public:
   static std::string MyTypeName() { return "float8_t"; }
   explicit RField(std::string_view name)
     : Detail::RFieldBase(name, MyTypeName(), ENTupleStructure::kLeaf, true /* isSimple */) {}
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
...
/// Dummy class to produce different template specialization for custom-bit float and double. Members can be added later if required.
class RCustomSizedFloat{ };
...
private:
   ...
   std::int64_t fMax;
public:
   std::string MyTypeName() { return "float(nBits(" + std::to_string(fNBits) + ")/min(" + std::to_string(fMin) + ")/max(" + std::to_string(fMax) + ")"; }
   explicit RField(std::string_view name, std::size_t nBits, std::int64_t min, std::int64_t max)
   : Detail::RFieldBase(name, "" /* fType */, ENTupleStructure::kLeaf, true /* isSimple */), fNBits{nBits}, fMin{min}, fMax{max} { SetType(MyTypeName()); } // TypeName relies on fNBits, etc. so it has to be set after initalizing fNBits, etc.
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
   ~RField() = default;
...
private:
   ...
   std::int64_t fMax;
public:
   std::string MyTypeName() { return "double(nBits(" + std::to_string(fNBits) + ")/min(" + std::to_string(fMin) + ")/max(" + std::to_string(fMax) + ")"; }
   explicit RField(std::string_view name, std::size_t nBits, std::int64_t min, std::int64_t max)
   : Detail::RFieldBase(name, "" /* MyTypeName() */, ENTupleStructure::kLeaf, true /* isSimple */), fNBits{nBits}, fMin{min}, fMax{max} { SetType(MyTypeName()); } // MyTypeName depends on fNBits, fMin and fMax, so these members should be initialized first.
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
   ~RField() = default;
   RFieldBase* Clone(std::string_view newName) final { return new RField(newName, fNBits, fMin, fMax); }
...
public:
   explicit RField(std::string_view name, std::size_t nBits, std::int64_t min, std::int64_t max)
   : RFieldArray(name, std::make_unique<RField<ItemT, RCustomSizedFloat>>(RField<ItemT>::MyTypeName() + "(nBits(" + std::to_string(nBits) + ")/min(" + std::to_string(min) + ")/max(" + std::to_string(max) + ")", nBits, min, max), N), fNBits{nBits}, fMin{min}, fMax{max}
   {}
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
   ~RField() = default;
...
public:
   explicit RField(std::string_view name, std::size_t nBits, std::int64_t min, std::int64_t max)
      : RFieldVector(name, std::make_unique<RField<ItemT, RCustomSizedFloat>>(RField<ItemT>::MyTypeName() + "(nBits(" + std::to_string(nBits) + ")/min(" + std::to_string(min) + ")/max(" + std::to_string(max) + ")", nBits, min, max)), fNBits{nBits}, fMin{min}, fMax{max}
   {}
   RField(RField&& other) = default;
   RField& operator =(RField&& other) = default;
   ~RField() = default;
